def get_timestep_data_iter(*data, batch_size, time_step, device, offset, **kwargs):
    ...
    for t in range(offset, total_timesteps - time_step - 2):
        for start_stock in range(0, total_stocks, batch_size):
            end_stock = start_stock + batch_size
            if end_stock > total_stocks:
                continue

            batch_T = T[t:t+time_step, start_stock:end_stock]
            batch_I = I[t:t+time_step, start_stock:end_stock]
            batch_X = X[t:t+time_step, start_stock:end_stock]
            batch_X_selected = X_selected[t:t+time_step, start_stock:end_stock]
            batch_msk = msk[t:t+time_step, start_stock:end_stock]
            ...
            if has_label:
                batch_label = label[t:t+time_step, start_stock:end_stock]
                batch_label = batch_label.permute(1, 0) if len(batch_label.shape) == 2 else batch_label.permute(1, 0, 2)

                y = batch_label
                y_msk = batch_msk
                X_day_1 = X_selected[t+1:t+time_step+1, start_stock:end_stock]
                msk_day_1 = msk[t+1:t+time_step+1, start_stock:end_stock]
                X_day_2 = X_selected[t+2:t+time_step+2, start_stock:end_stock]
                msk_day_2 = msk[t+2:t+time_step+2, start_stock:end_stock]
